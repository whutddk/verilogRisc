...
set_property -dict { PACKAGE_PIN W19    IOSTANDARD LVCMOS33 } [get_ports { i_sysclk }]; 
create_clock -add -name sys_clk_pin -period 10.00 -waveform {0 5} [get_ports {i_sysclk}];

set_property -dict { PACKAGE_PIN Y18    IOSTANDARD LVCMOS33 } [get_ports { i_rtcclk }]; 
create_clock -add -name sys_clk_pin -period 30517.58 -waveform {0 15258.79} [get_ports {i_rtcclk}];
